...
export const sendDeliveryOtp = async (req, res) => {
    try {
        ...
        try {
            ...
            console.error("❌ sendDeliveryOtp: Mail delivery failed but OTP was saved. OTP is:", otp, mailError);
            emailError = mailError.message;
        }

        if (emailError) {
            return res.status(200).json({ 
                message: `OTP generated (Email delivery failed. You can bypass using code: FIREBASE_SUCCESS)` 
            })
        }
