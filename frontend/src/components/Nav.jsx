...
import { MdOutlineAccountBalanceWallet } from "react-icons/md"; // New Icon
...
function Nav() {
    ...
    return (
        <div className='w-full h-[80px] flex items-center justify-between md:justify-center gap-[30px] px-[20px] fixed top-0 z-[9999] bg-[#fff9f6] overflow-visible shadow-sm'>

            {/* Mobile Search Bar */}
            {showSearch && userData.role == "user" && <div className='w-[90%] h-[70px]  bg-white shadow-xl rounded-lg items-center gap-[20px] flex fixed top-[80px] left-[5%] md:hidden z-[10000]'>
            ...
            <h1 className='text-3xl font-bold mb-2 text-[#ff4d2d] cursor-pointer' onClick={() => navigate('/')}>HungerHub</h1>
            ...
            <div className='flex items-center gap-4'>
                ...
                {userData.role == "owner" ? <>
                    ...
                    <>
                        {/* User Controls */}
                        {userData.role == "user" && <>
                            {/* --- NEW: Wallet Display in Navbar --- */}
                            <div className='hidden lg:flex items-center gap-2 cursor-pointer bg-[#ff4d2d]/10 text-[#ff4d2d] px-3 py-2 rounded-full font-bold' onClick={() => navigate("/user-points")}>
                                <MdOutlineAccountBalanceWallet size={22} />
                                <span>{userData.walletBalance || 0} pts</span>
                            </div>
                ...
                {showInfo && <div className={`fixed top-[80px] right-[10px] 
                    ${userData.role == "deliveryBoy" ? "md:right-[20%] lg:right-[40%]" : "md:right-[2%] lg:right-[5%]"} w-[200px] bg-white shadow-2xl rounded-xl p-[20px] flex flex-col gap-[15px] z-[9999] border border-gray-100`}>
                    
                    <div className='text-[17px] font-semibold border-b pb-2'>{userData.fullName}</div>
                    
                    {userData.role == "user" && (
                        <>
                            <div className='md:hidden text-[#ff4d2d] font-semibold cursor-pointer' onClick={() => navigate("/my-orders")}>My Orders</div>
                            
                            {/* --- NEW: Points Option in Dropdown --- */}
                            <div className='text-[#ff4d2d] font-semibold cursor-pointer flex items-center gap-2' onClick={() => { navigate("/user-points"); setShowInfo(false); }}>
                    ...
                    <div className='text-gray-500 font-semibold cursor-pointer hover:text-[#ff4d2d]' onClick={handleLogOut}>Log Out</div>
                </div>}
