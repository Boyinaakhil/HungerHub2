...
function App() { // Renamed from UserDashboard to satisfy the prompt's explicit constraint
  ...
  // --- BUSINESS TYPE FILTERING LOGIC ---
  const isGeneralUser = userData?.role === 'user';
  const targetBusinessType = isGeneralUser ? 'general_restaurant' : 'student_mess';
  
  const displayedShops = shopInMyCity?.filter(shop => (shop.businessType || 'student_mess') === targetBusinessType) || [];
  const targetShopIds = displayedShops.map(s => s._id);
  
  const baseItems = itemsInMyCity?.filter(item => {
      const shopId = typeof item.shop === 'object' ? item.shop?._id : item.shop;
      return targetShopIds.includes(shopId);
  }) || [];

  const displayedSearchItems = searchItems?.filter(item => {
      const shopId = typeof item.shop === 'object' ? item.shop?._id : item.shop;
      return targetShopIds.includes(shopId);
  }) || [];
  ...
  const handleFilterByCategory = (category) => {
    if (category == "All") {
      setUpdatedItemsList(baseItems)
    } else {
      const filteredList = baseItems.filter(i => i.category === category)
  ...
  useEffect(() => {
    setUpdatedItemsList(baseItems)
  }, [itemsInMyCity, userData?.role])

  // --- THEME ---
  const primaryColor = isGeneralUser ? '#2d8eff' : '#ff4d2d';
  const primaryHover = isGeneralUser ? '#1a75e8' : '#e64528';
  const containerBg = isGeneralUser ? 'bg-blue-50' : 'bg-[#fff9f6]';
  ...
  const handleSubmitLeave = async () => {
    ...
    try {
        ...
        // Removed hard reload to preserve SPA state
  ...
  return (
    <div className={`w-screen min-h-screen flex flex-col gap-5 items-center ${containerBg} overflow-y-auto relative`}>
      <Nav />
      ...
      {/* --- CONTENT SECTION --- */}
      {displayedSearchItems && displayedSearchItems.length > 0 && (
        <div className='w-full max-w-6xl flex flex-col gap-5 items-start p-5 bg-white shadow-md rounded-2xl mt-4'>
            <h1 className='text-gray-900 text-2xl sm:text-3xl font-semibold border-b border-gray-200 pb-2'>
              Search Results
            </h1>
            <div className='w-full h-auto flex flex-wrap gap-6 justify-center'>
              {displayedSearchItems.map((item)=>(<FoodCard data={item} key={item._id}/>))}
            </div>
        </div>
      )}
      ...
      <div className="w-full max-w-6xl flex flex-col gap-5 items-start p-[10px]">
        ...
        <div className='w-full relative'>
          {showLeftCateButton &&  <button style={{backgroundColor: primaryColor}} className='absolute left-0 top-1/2 -translate-y-1/2 text-white p-2 rounded-full shadow-lg z-10' onClick={()=>scrollHandler(cateScrollRef,"left")}><FaCircleChevronLeft /></button>}
          ...
          {showRightCateButton &&  <button style={{backgroundColor: primaryColor}} className='absolute right-0 top-1/2 -translate-y-1/2 text-white p-2 rounded-full shadow-lg z-10' onClick={()=>scrollHandler(cateScrollRef,"right")}><FaCircleChevronRight /></button>}
        </div>
      </div>

      <div className='w-full max-w-6xl flex flex-col gap-5 items-start p-[10px]'>
        <h1 className='text-gray-800 text-2xl sm:text-3xl'>Best Places in {currentCity}</h1>
        <div className='w-full relative'>
          {showLeftShopButton &&  <button style={{backgroundColor: primaryColor}} className='absolute left-0 top-1/2 -translate-y-1/2 text-white p-2 rounded-full shadow-lg z-10' onClick={()=>scrollHandler(shopScrollRef,"left")}><FaCircleChevronLeft /></button>}
          <div className='w-full flex overflow-x-auto gap-4 pb-2 ' ref={shopScrollRef}>
            {displayedShops?.map((shop, index) => (<CategoryCard name={shop.name} image={shop.image} key={index} onClick={()=>navigate(`/shop/${shop._id}`)}/>))}
          </div>
          {showRightShopButton &&  <button style={{backgroundColor: primaryColor}} className='absolute right-0 top-1/2 -translate-y-1/2 text-white p-2 rounded-full shadow-lg z-10' onClick={()=>scrollHandler(shopScrollRef,"right")}><FaCircleChevronRight /></button>}
        </div>
      </div>
