...
function FoodCard({ data }) {
    ...
    const cartItem = cartItems.find(item => (item.id === data._id || item._id === data._id));
    ...
    const handleIncrease = () => {
        if (cartItem) {
            // If in cart, update Redux immediately for persistence
            ...
            // If not in cart, update local state only
    ...
    const handleDecrease = () => {
        if (cartItem) {
            // If in cart, update Redux immediately
    ...
    const handleAddToCart = () => {
        // Only trigger Add to Cart if it's NOT in the cart yet. 
        // If it IS in the cart, the +/- buttons already handled the updates.
        if (!cartItem && quantity > 0) {
            dispatch(addToCart({
                id: data._id,
                _id: data._id, // Ensure ID is passed for consistency
                name: data.name,
                price: data.price,
                image: data.image,
                shop: data.shop,
        ...
        <div className='w-[250px] rounded-2xl border-2 border-[#ff4d2d] bg-white shadow-md overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-col'>
            ...
            <div className='flex items-center justify-between mt-auto p-3'>
                ...
                <div className='flex items-center border rounded-full overflow-hidden shadow-sm'>
                    ...
                    <button 
                        className={`${cartItem ? "bg-gray-800 cursor-default" : "bg-[#ff4d2d] hover:bg-[#e04328]"} text-white px-3 py-2 transition-colors`}
                        onClick={handleAddToCart}
                        disabled={!!cartItem} // Optional: Disable button if already in cart to prevent double-add confusion
                    >
