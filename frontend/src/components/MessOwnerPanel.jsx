...
function OwnerDashboard() {
  ...
  const isStudentMess = myShopData?.businessType === 'student_mess';
        ...
        <div className='w-full max-w-6xl flex flex-col gap-6 px-4 sm:px-6 pt-[30px]'>
          ...
          {activeTab === "dashboard" && (
            <div className="space-y-6 animate-fade-in">
                <div className={`grid grid-cols-1 ${isStudentMess ? 'md:grid-cols-3' : 'md:grid-cols-2'} gap-4`}>
                    ...
                    <div className="bg-white p-5 rounded-xl shadow-sm border-l-4 border-orange-500 flex items-center justify-between">
                        <div>
                            <p className="text-gray-500 text-xs font-bold uppercase">{isStudentMess ? "Cash Collected" : "Total Revenue"}</p>
                ...
                <div>
                    ...
                    {orders.length > 0 ? (
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            {orders.map((order) => (
                                <OwnerOrderCard key={order._id} data={order} refreshOrders={fetchOwnerOrders} isStudentMess={isStudentMess} />
                            ))}
                        </div>
                    ) : (
                                  ...
                                  >
                                      ...
                                      <div className="text-right">
                                          <div className="flex gap-4 text-sm">
                                              {isStudentMess && <span className="text-green-600 font-bold flex items-center gap-1"><FaWallet/> ₹{day.points}</span>}
                                          ...
                                          <table className="w-full text-left text-sm">
                                              <thead className="text-gray-500 border-b border-gray-200">
                                                  <tr>
                                                      ...
                                                      {isStudentMess && <th className="pb-2 font-semibold text-right">Points</th>}
                                                      {isStudentMess && <th className="pb-2 font-semibold text-right">Cash</th>}
                                              ...
                                              <tbody className="divide-y divide-gray-200">
                                                  {day.details.map((order, i) => (
                                                      <tr key={i}>
                                                          ...
                                                          {isStudentMess && <td className="py-2 text-right text-green-600 font-bold">₹{order.points}</td>}
                                                          {isStudentMess && <td className="py-2 text-right text-orange-500 font-bold">₹{order.cash}</td>}
